...
[2, "tk_global_message", {(_this select 0) call TKfSystemChat}] call TKNetAddEvent;
[2, "tk_local_message", {if (local (_this select 0)) then {(_this select 1) call TKfSystemChat}}] call TKNetAddEvent;
[2, "tk_end_mission", {if (local (_this select 0)) then {endMission "LOSER"; titleCut ["Mission Protection System: You was punished cause you are a stupid noob or cause you are only stupid.","black faded", 0];}}] call TKNetAddEvent;
[2, "tk_freezing", {if (local (_this select 0)) then {
...
0 spawn {
	...
	if (objNull == TcB_Ref_Point) then {hintC "ERROR: You have to define an object called 'TcB_Ref_Point' to define the base protection zone."};
