...
if (ctrlText _answer != "") then {
	...
	if (_needed == ctrlText _answer) then {
		...
		"Captcha passed" call TKfSystemChat;
		...
		switch (_num) do {
			case (1) : {"Captcha failed - 2 times left" call TKfSystemChat; [] execVM tk_path + "dialogs\create_captcha.sqf";};
			case (2) : {"Captcha failed - 1 times left" call TKfSystemChat; [] execVM tk_path + "dialogs\create_captcha.sqf";};
			case (3) : {
				"Captcha limit reached!" call TKfSystemChat;
				sleep 1;
				closeDialog 0;
				sleep 0.1;
	...
	"Error - missing input" call TKfSystemChat;
};