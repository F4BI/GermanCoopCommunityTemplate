...
private ["_center_type","_cnt","_handle","_grp_type","_a_radius","_center","_i", "_num"];

_center_type = _this select 0;
_center = switch (toUpper (typeName _center_type)) do {
	case ("OBJECT") : {position _center_type};
	case ("STRING") : {getMarkerPos _center_type};
	case ("ARRAY") : {_center_type};
};
_cnt = count _this;
...
for "_i" from 1 to (_cnt - 1) do {
	...
	if (_grp_type in ["basic","specops","motor_basic","motor_at","motor_aa","mech_basic","mech_at","mech_aa","mech_support","tank_section","apc_section","tank_aa_platoon"]) then {
		_a_radius = if (count _handle > 1) then {_handle select 1} else {100};
