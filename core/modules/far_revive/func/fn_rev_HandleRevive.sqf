...
if (isPlayer _healer) then {
	[_healer, _injuredperson] call tcb_fnc_rev_medicEquipment;
};
...
_skill_factor = if (_healer call tcb_fnc_rev_isMedic) then {40+(random 10)} else {70+(random 10)};
...
while {
	...
	sleep 0.5;
	if (isPlayer _healer) then {["Applying First Aid",((time - _time) / (_damage)) min 1] spawn tcb_fnc_rev_progressBar};
};
...
call tcb_fnc_rev_garbage;

_old_damage = damage _injuredperson;

if (!tcb_healerStopped) then {
	_isMedic = _healer call tcb_fnc_rev_isMedic;
