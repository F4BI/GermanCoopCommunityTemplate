...
_skill_factor = if (_healer call tcb_fnc_rev_isMedic) then {10+(random 5)} else {20+(random 10)};
...
while {
	...
	sleep 0.5;
	if (isPlayer _healer) then {["Try to stabilize",((time - _time) / (_damage)) min 1] spawn tcb_fnc_rev_progressBar};
};
