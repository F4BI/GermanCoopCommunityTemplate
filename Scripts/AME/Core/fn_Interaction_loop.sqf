/*
    ...
    * Should only be called once per mission.
	...
	Example:
	true spawn AME_Core_fnc_Interaction_loop;
*/

_lastTarget = cursorTarget;
{
    _onObject = _x select 0;
    _objActions = _lastTarget getVariable ["AME_Core_Interaction_Actions", []];
    if (!(_x in _objActions)) then {
        if (typeName _onObject == "STRING") then {
            if (_lastTarget isKindOf _onObject) then {
                _lastTarget addAction [_x select 1, _x select 3, _x select 4, 1.5, true, true, "", _x select 2];
                _objActions pushBack _x;
                _lastTarget setVariable ["AME_Core_Interaction_Actions", _objActions];
            };
        };
...
} count AME_Core_Interaction_Actions;
